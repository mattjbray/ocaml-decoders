...
type root = { main : string; trees : (string * node) list }

and node =
  | Action of { id : string; attrs : (string * string) list }
  | Sequence of node list
  | Subtree of { id : string }

let rec pp_root fmt root =
  fprintf fmt "@[<1>(:main %S@ :trees @[<1>(%a)@])@]" root.main
...
and pp_node fmt = function
  | Action { id; attrs = _ } -> fprintf fmt "@[<1>(:action %S)@]" id
  | Sequence nodes ->
      fprintf fmt "@[<1>(:sequence@ @[<1>(%a)@])@]"
        (list ~sep:(return "@ ") pp_node)
        nodes
  | Subtree { id } -> fprintf fmt "@[<1>(:subtree %S)@]" id

open Decoders_ezxmlm.Decode

let rec node node_ty : node decoder =
  match node_ty with
  | "Action" -> attr "ID" >>= fun id -> succeed (Action { id; attrs = [] })
  | "Sequence" ->
      pick_children (any_tag >>= fun node_ty -> pure (node node_ty))
      >>= fun nodes -> succeed (Sequence nodes)
  | "Subtree" -> attr "ID" >>= fun id -> succeed (Subtree { id })
  | _ -> fail "Unknown node type"

let tree : (string * node) decoder =
  attr "ID" >>= fun id ->
  pick_children (any_tag >>= fun node_ty -> pure (node node_ty)) >>= function
  | [ node ] -> succeed (id, node)
  | _ -> fail "Expected a single child"

let root : root decoder =
  tag "root" >>= fun () ->
  attr "main_tree_to_execute" >>= fun main ->
  pick_children (tag "BehaviorTree" >>= fun () -> pure tree) >>= fun trees ->
  succeed { main; trees }
...
let () =
  ...
  printf "@[<v 2>Comments are skipped:@ %a@]@.@."
...
let root_decoder =
  ...
  pick_children
    ( any_tag >>= fun nm ->
      match nm with
      | "node2" -> tag "node2" >>= fun () -> succeed @@ node2_decoder
      | _ -> fail "invalid node" )
  >|= List.concat
...
let () =
  ...
  | Ok fld ->
      printf "%a"
        ...
        fld (* this prints "More data, even more data" *)
