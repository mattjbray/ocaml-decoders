...
let decoders_suite =
  ...
  ; "field_opt present" >::
    decoder_test
      ...
      ~input:(m_to_str (M.Map [(M.String "optional", M.String "hello")]))
      ~expected:(Some "hello")
  ; "field_opt missing" >::
    decoder_test
      ~decoder:(field_opt "optional" string)
      ~input:(m_to_str (M.Map [(M.String "missing", M.String "hello")]))
      ~expected:None
  ...
  ; "int64 overflow" >::
    begin fun _ ->
      match
        decode_string int (m_to_str (M.Int64 Int64.max_int))
      with
      | Ok v -> assert_string (Printf.sprintf "expected decode error, got %d" v)
      | Error _e -> ()
    end
  ]
